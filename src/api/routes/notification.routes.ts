...
const router = Router();
const getNotiController = (): NotificationController =>
  container.get<NotificationController>(TYPES.NotificationController);
...
router.post(
  '/',
  requireAuth,
  asyncHandler((req, res) => getNotiController().sendNotification(req, res)),
);

router.get(
  '/',
  requireAuth,
  asyncHandler((req, res) => getNotiController().listNotifications(req, res)),
);
router.post(
  '/:notificationId/read',
  requireAuth,
